		...
		if e.inst.Parent then
			...
			if ok then
				if e.disabledByUs and enabled then
					-- Something else changed our object; release ownership instead of fighting it.
...
local function collectCandidates(wantEnabledOnes)
	...
	for _, e in ipairs(registry) do
		if e.inst.Parent and e.originalValue == true then
			local ok, enabled = pcall(function() return e.inst.Enabled end)
			if ok then
				if wantEnabledOnes and enabled and not e.disabledByUs then
					table.insert(list, e)
				elseif not wantEnabledOnes and e.disabledByUs and not enabled then
					table.insert(list, e)
				end
			end
		end
...
local function getTopInsightsData(n)
	local scored = {}
	for _, e in ipairs(registry) do if e.sampleCount > 0 and e.inst.Parent then table.insert(scored, e) end end
