-- CokeBoys-APE Universal v4 | dev: Syntax_errrror
...
local CONFIG = {
	...
	ExperimentMaxFrameMs = 55,
	ExperimentSpikeRatio = 1.65,
	ExperimentCooldownAfterAbort = 4.0,
	ShadowBudgetPerStep = 24,
	ShadowDisableStart = 0.72,
	ShadowDisableFull = 0.94,
	WorkloadRefreshInterval = 8,
	RecoveryHysteresis = 0.985,
	ExternalChangeCooldown = 3.0,
	CriticalVisualCooldown = 12.0,
	MinPersistentConfidence = 0.58,
	MinPersistentSamples = 2,
	VisualCueClasses = {Beam = true, Trail = true},
}

-- Choose the device profile before the runtime state is built.
-- Touch without a keyboard is treated as mobile.
local IS_MOBILE = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled

if IS_MOBILE then
	CONFIG.SampleWindow = 0.5
	CONFIG.HistoryLength = 60
	CONFIG.FrameRingSize = 60
	CONFIG.SpikeMultiplier = 3.5
	CONFIG.SpikeMinDelta = 0.05
	CONFIG.SpikeCooldown = 0.75
	CONFIG.AttackRate = 1.05
	CONFIG.DecayRate = 0.12
	CONFIG.SpikeJump = 0.10
	CONFIG.RecoveryMargin = 1.06
	CONFIG.DefaultTargetFPS = 60
	CONFIG.MinTargetFPS = 30
	CONFIG.MaxTargetFPS = 120
	CONFIG.StreamingRadiusFloor = 128
	CONFIG.StreamingStart = 0.82
	CONFIG.StreamingMinRatio = 0.70
	CONFIG.DecisionInterval = 1.75
	CONFIG.SettleTime = 1.0
	CONFIG.MaxBatch = 1
	CONFIG.JitterSpikyThreshold = 12
	CONFIG.BaselineSamples = 5
	CONFIG.SettleSamples = 5
	CONFIG.ExperimentSampleInterval = 0.25
	CONFIG.MinImprovementMs = 0.40
	CONFIG.NoiseFloorMs = 0.15
	CONFIG.ConfidenceAlpha = 0.14
	CONFIG.MaxExperimentsPerObject = 5
	CONFIG.ExperimentCooldown = 1.5
	CONFIG.ContextTolerance = 0.18
	CONFIG.WarmupTime = 3.0
	CONFIG.MaxFrameDelta = 0.25
	CONFIG.ControlInterval = 0.08
	CONFIG.RegistryRefreshInterval = 5
	CONFIG.MetricsInterval = 0.5
	CONFIG.P95Interval = 0.75
	CONFIG.MinUsefulCost = 2.5
	CONFIG.MinDisableIntensity = 0.04
	CONFIG.RecoveryHysteresis = 0.98
else
	CONFIG.DefaultTargetFPS = 240
	CONFIG.MinTargetFPS = 30
	CONFIG.MaxTargetFPS = 400
end
...
local state = {
	...
	workload = {partPressure = 0, physicsPressure = 0, scriptPressure = 0, visualPressure = 0, likelyNonVisual = 0},
	ping = 0,
	intensity = 0,
...
local function trackWorkload(inst, delta)
	local class = inst.ClassName
	if class == "BasePart" then
		workload.parts = math.max(0, workload.parts + delta)
		local ok, anchored = pcall(function() return inst.Anchored end)
		if ok and not anchored then
			workload.unanchoredParts = math.max(0, workload.unanchoredParts + delta)
		end
	elseif class == "Script" or class == "LocalScript" then
		workload.scripts = math.max(0, workload.scripts + delta)
	elseif class == "ModuleScript" then
		workload.modules = math.max(0, workload.modules + delta)
	elseif KIND_BASE_WEIGHT and KIND_BASE_WEIGHT[class] then
		workload.particleLike = math.max(0, workload.particleLike + delta)
	end
end
...
local function isProtectedVisual(inst)
	if not inst or not inst.Parent then
		return true
	end

	local ok, protected = pcall(function()
		return inst:GetAttribute("CokeBoysProtected") == true
			or inst:GetAttribute("CokeBoysKeepVisual") == true
			or inst:GetAttribute("GameplayCritical") == true
	end)
	...
	if playerCharacter and inst:IsDescendantOf(playerCharacter) then
		return true
	end
...
local function removeEntry(entry)
	...
	if entry.disabledByUs then
		state.disabledCount = math.max(0, state.disabledCount - 1)
...
local function registerInstance(inst)
	...
	local entry = {
		...
		protected = isProtectedVisual(inst),
		externallyManagedUntil = 0,
...
-- Build the object list in small chunks so large maps do not stall startup.
local registryReady = false
local potatoSyncNeeded = false
Workspace.DescendantAdded:Connect(function(inst)
	trackWorkload(inst, 1)
...
task.spawn(function()
	...
	local index = 1
	local batchSize = IS_MOBILE and 80 or 160

	while index <= #list do
		local stop = math.min(index + batchSize - 1, #list)
		for i = index, stop do
			trackWorkload(list[i], 1)
			registerInstance(list[i])
		end
		index = stop + 1
		task.wait()
	end
	...
	registryReady = true
	markCandidatesDirty()
	potatoSyncNeeded = potatoModeActive
end)
...
local function cachePostEffect(inst)
	...
	if ok then
		postEffects[inst] = {original = enabled, managed = false, protected = isProtectedVisual(inst), externalUntil = 0}
	end
end
...
local globalShadowsExternalUntil = 0
local streamingReducedByUs = false
local streamingExternalUntil = 0
...
local function rebuildCandidates()
	...
	for _, e in ipairs(registry) do
		if e.inst.Parent and e.originalValue == true and not e.protected and now >= (e.cooldownUntil or 0) and now >= (e.externallyManagedUntil or 0) then
			local ok, enabled = pcall(function()
				return e.inst.Enabled
			end)
...
local function computeFinalRank(e)
	if not e.inst.Parent or e.originalValue ~= true or e.protected then
		return -math.huge
	end

	if CONFIG.VisualCueClasses[e.kind] and e.sampleCount < 1 then
		return -math.huge
	end
...
local function updateWorkloadProfile()
	workload.lastRefresh = os.clock()
	local partPressure = math.clamp(workload.parts / (IS_MOBILE and 9000 or 25000), 0, 1)
	local physicsPressure = math.clamp(workload.unanchoredParts / (IS_MOBILE and 1200 or 4000), 0, 1)
	local scriptPressure = math.clamp((workload.scripts + workload.modules * 0.35) / (IS_MOBILE and 180 or 700), 0, 1)
	local visualPressure = math.clamp(workload.particleLike / (IS_MOBILE and 250 or 900), 0, 1)
	state.workload = {
		partPressure = partPressure,
		physicsPressure = physicsPressure,
		scriptPressure = scriptPressure,
		visualPressure = visualPressure,
		likelyNonVisual = math.clamp(math.max(physicsPressure, scriptPressure) - visualPressure * 0.35, 0, 1),
	}
end

updateWorkloadProfile()

local function testingIsSafe()
	if state.warmupTimer < CONFIG.WarmupTime or state.actionState ~= "idle" then
		return false
	end
	if state.frameTimeMs > CONFIG.ExperimentMaxFrameMs then
		return false
	end
	if state.p95FrameTimeMs > math.max(state.frameTimeMs * CONFIG.ExperimentSpikeRatio, 0) then
		return false
	end
	if state.diagnosis ~= "stable" or state.stability < 0.75 then
		return false
	end
	if state.workload and state.workload.likelyNonVisual > 0.72 then
		return false
	end
	return true
end
...
local function contextSignature()
	return {
		...
		nonVisual = state.workload and state.workload.likelyNonVisual or 0,
		ping = state.ping,
	}
end
...
local function contextDistance(a, b)
	...
	local nonVisual = math.abs((a.nonVisual or 0) - (b.nonVisual or 0))
	local ping = math.abs((a.ping or 0) - (b.ping or 0)) / 100
...
local function applyEntry(e, enabled)
	...
	local now = os.clock()
	if not enabled and e.protected then
		return false
	end
	if not enabled and now < (e.externallyManagedUntil or 0) then
		return false
	end
	...
	-- Never fight a game script that changed the value after we touched it.
	if e.disabledByUs and current ~= false and current ~= enabled then
		e.disabledByUs = false
		e.lastObservedEnabled = current
		e.externallyManagedUntil = now + CONFIG.ExternalChangeCooldown
		markCandidatesDirty()
		return false
	end
	...
	if success then
		...
		if wasDisabled ~= e.disabledByUs then
			state.disabledCount += e.disabledByUs and 1 or -1
		end
		markCandidatesDirty()
	end
...
local function startAction(batch, direction, label)
	if pendingAction or #batch == 0 or not testingIsSafe() then
		return false
	end
	...
	if not e or not e.inst.Parent or e.originalValue ~= true or e.protected then
		return false
	end
	...
	if direction == "disable" and CONFIG.VisualCueClasses[e.kind] and e.sampleCount < 1 then
		return false
	end
...
local function resolveAction(aborted)
	...
	if aborted then
		...
		if e then
			e.lastResult = "test aborted: frame budget"
			e.failCount += 1
			e.cooldownUntil = os.clock() + CONFIG.ExperimentCooldownAfterAbort
		end
		pendingAction = nil
	...
	if action.direction == "disable" then
		local persistent = meaningful
			and e.sampleCount >= CONFIG.MinPersistentSamples
			and e.confidence >= CONFIG.MinPersistentConfidence
		...
		elseif meaningful then
			e.successCount += 1
			e.lastResult = ("promising +%.2fms — needs confirmation"):format(delta)
			if e.inst.Parent then
				applyEntry(e, e.originalValue)
			end
			e.cooldownUntil = os.clock() + 1.5
			addLog(("RETEST LATER %s — +%.2fms, evidence %d/%d"):format(entryName(e), delta, e.sampleCount, CONFIG.MinPersistentSamples), "info")
		else
			e.failCount += 1
...
local function updateExperiment(dt)
	...
	if state.frameTimeMs > CONFIG.ExperimentMaxFrameMs or (state.p95FrameTimeMs > math.max(action.baseline * CONFIG.ExperimentSpikeRatio, 1)) then
		resolveAction(true)
		addLog("Test stopped to protect frame time", "warn")
		return
	end
...
local function runDecisionCycle()
	if potatoModeActive or not registryReady then return end
	updateDiagnosis()
	...
	if state.workload and state.workload.likelyNonVisual > 0.82 and state.diagnosis == "sustained" then
		-- The visible workload is probably not the main bottleneck.
		-- Hold the visual optimizer instead of stripping more effects.
		state.intensity = math.min(state.intensity, 0.35)
		return
	end

	local eligibleCount = #getEnabledCandidates() + #getDisabledCandidates()
	...
	local function pickBest(candidates, restore)
		local best, bestScore = nil, restore and math.huge or -math.huge
		for _, candidate in ipairs(candidates) do
			local score
			if restore then
				score = candidate.learnedImpact * math.max(candidate.confidence, 0.25)
				if score < bestScore then best, bestScore = candidate, score end
			else
				score = computeFinalRank(candidate)
				if score > bestScore then best, bestScore = candidate, score end
			end
		end
		return best
	...
	if desiredCount > currentDisabled then
		local candidate = pickBest(getEnabledCandidates(), false)
		if candidate then
			startAction({candidate}, "disable", "performance check")
		end
	elseif desiredCount < currentDisabled
		and state.intensity <= CONFIG.RecoveryHysteresis then
		local candidate = pickBest(getDisabledCandidates(), true)
		if candidate then
			startAction({candidate}, "enable", "restore check")
		end
	end
end
...
local shadowAppliedCount = 0

local function registerShadowPart(inst)
	if inst.ClassName ~= "BasePart" or shadowLookup[inst] then return end
	local ok, cast = pcall(function() return inst.CastShadow end)
	if not ok or not cast then return end
	local e = {inst = inst, original = cast, index = #shadowEntries + 1, managed = false, protected = isProtectedVisual(inst)}
	shadowEntries[#shadowEntries + 1] = e
	shadowLookup[inst] = e
end

local function removeShadowPart(inst)
	local e = shadowLookup[inst]
	if not e then return end
	shadowLookup[inst] = nil
	local index = e.index
	local last = shadowEntries[#shadowEntries]
	if index and shadowEntries[index] == e then
		if last ~= e then
			shadowEntries[index] = last
			last.index = index
		end
		shadowEntries[#shadowEntries] = nil
	end
end

local function stepPartShadows(intensity)
	if #shadowEntries == 0 then return end
	local fraction = 0
	if intensity >= CONFIG.ShadowDisableStart then
		fraction = math.clamp((intensity - CONFIG.ShadowDisableStart) / math.max(CONFIG.ShadowDisableFull - CONFIG.ShadowDisableStart, 0.01), 0, 1)
	end
	local desired = math.floor(#shadowEntries * fraction + 0.5)
	local changed = 0

	-- Only touch entries that are not already managed by us.
	while changed < CONFIG.ShadowBudgetPerStep and shadowCursor <= desired do
		local e = shadowEntries[shadowCursor]
		if e and e.inst.Parent and not e.protected then
			local ok, current = pcall(function() return e.inst.CastShadow end)
			if ok then
				if e.managed and current ~= false then
					-- Game code reclaimed this property; leave it alone.
					e.managed = false
				else
					if current ~= false then
						pcall(function() e.inst.CastShadow = false end)
					end
					e.managed = true
				end
			end
	...
	if desired < shadowAppliedCount then
		for i = 1, #shadowEntries do
			local e = shadowEntries[i]
			if e.inst.Parent then
				if i <= desired and not e.protected then
					local ok, current = pcall(function() return e.inst.CastShadow end)
					if ok and (not e.managed or current == false) then
						pcall(function() e.inst.CastShadow = false end)
						e.managed = true
					end
				elseif e.managed then
					pcall(function() e.inst.CastShadow = e.original end)
					e.managed = false
				end
			end
		end
		shadowAppliedCount = desired
		shadowCursor = desired + 1
	else
		shadowAppliedCount = math.min(desired, #shadowEntries)
	end
end

for _, inst in ipairs(Workspace:GetDescendants()) do registerShadowPart(inst) end
Workspace.DescendantAdded:Connect(registerShadowPart)
Workspace.DescendantRemoving:Connect(removeShadowPart)
...
local function forceRestoreAll()
	...
	if postEffectsDisabledByUs then
		for inst, data in pairs(postEffects) do
			if inst.Parent and data.managed then
				local ok, current = pcall(function() return inst.Enabled end)
				if ok and current == false then
					pcall(function() inst.Enabled = data.original end)
				end
				data.managed = false
			end
		end
	...
	if shadowsDisabledByUs then
		local ok, current = pcall(function() return Lighting.GlobalShadows end)
		if ok and current == false then
	...
	if streamingReducedByUs then
		local ok, current = pcall(function() return Workspace.StreamingTargetRadius end)
		if ok and current == appliedStreamingRadius then
	...
	for _, e in ipairs(shadowEntries) do
		if e.inst.Parent and e.managed then
			local ok, current = pcall(function() return e.inst.CastShadow end)
			if ok and current == false then
				pcall(function() e.inst.CastShadow = e.original end)
			end
	...
	shadowCursor = 1
	shadowAppliedCount = 0
	state.intensity = 0
	markCandidatesDirty()
end

local function applyGlobalEffects(intensity)
	-- If the game changes a global setting while we own it, release control.
	local now = os.clock()
	if shadowsDisabledByUs then
		local ok, current = pcall(function() return Lighting.GlobalShadows end)
		if ok and current ~= false then
			shadowsDisabledByUs = false
			globalShadowsExternalUntil = now + CONFIG.ExternalChangeCooldown
		end
	end
	...
	if streamingReducedByUs then
		...
		if ok and current ~= appliedStreamingRadius then
			streamingReducedByUs = false
			streamingExternalUntil = now + CONFIG.ExternalChangeCooldown
			appliedStreamingRadius = nil
		end
	end

	stepPartShadows(intensity)

	if intensity >= 0.5 and not postEffectsDisabledByUs then
		for inst, data in pairs(postEffects) do
			if inst.Parent and not data.protected and now >= (data.externalUntil or 0) then
				local ok, current = pcall(function() return inst.Enabled end)
				if ok and current == data.original then
					pcall(function() inst.Enabled = false end)
					data.managed = true
				elseif ok and data.managed and current ~= false then
					data.managed = false
					data.externalUntil = now + CONFIG.ExternalChangeCooldown
				end
			end
		end
	...
	if intensity >= 0.86 and state.diagnosis == "sustained" and not shadowsDisabledByUs and now >= globalShadowsExternalUntil then
		pcall(function() Lighting.GlobalShadows = false end)
		shadowsDisabledByUs = true
	elseif (intensity < 0.80 or state.diagnosis ~= "sustained") and shadowsDisabledByUs then
		pcall(function() Lighting.GlobalShadows = originalGlobalShadows end)
		shadowsDisabledByUs = false
	end

	if intensity >= CONFIG.StreamingStart and originalStreamingRadius > CONFIG.StreamingRadiusFloor and not streamingReducedByUs and now >= streamingExternalUntil then
		-- Keep streaming changes gradual and bounded so geometry does not suddenly disappear.
		local t = math.clamp((intensity - CONFIG.StreamingStart) / math.max(1 - CONFIG.StreamingStart, 0.01), 0, 1)
		local ratioFloor = math.max(CONFIG.StreamingMinRatio, CONFIG.StreamingRadiusFloor / math.max(originalStreamingRadius, 1))
		local targetRatio = 1 - t * (1 - ratioFloor)
		local target = math.floor(originalStreamingRadius * targetRatio)
	...
	elseif intensity < 0.6 and streamingReducedByUs then
		local ok, current = pcall(function() return Workspace.StreamingTargetRadius end)
		if ok and current == appliedStreamingRadius then
			pcall(function() Workspace.StreamingTargetRadius = originalStreamingRadius end)
		end
...
local function applyPotatoMode()
	...
	for _, e in ipairs(registry) do
		if e.inst.Parent and e.originalValue == true and not e.protected then
			applyEntry(e, false)
		end
	end
...
RunService.Heartbeat:Connect(function(dt)
	...
	if potatoModeActive and potatoSyncNeeded and registryReady then
		for _, e in ipairs(registry) do
	...
	local stableTarget = math.max(state.targetFPS, 1)
	local p95Ratio = state.p95FrameTimeMs / math.max(1000 / stableTarget, 0.1)
	local stableNow = state.jitterMs <= CONFIG.JitterSpikyThreshold and p95Ratio < 1.8 and state.diagnosis ~= "sustained"
	state.stability = math.clamp(state.stability + (stableNow and 0.08 or -0.16), 0, 1)

	state.warmupTimer += dt
	state.controlTimer += dt
	state.p95Timer += dt
	state.metricsTimer += dt
	state.workloadTimer += dt
	...
	if frameRingCount >= 8
		...
		if now - state.lastSpikeAt >= CONFIG.SpikeCooldown then
			...
			if state.autoOptimize
				...
				and state.warmupTimer >= CONFIG.WarmupTime
				and (not state.workload or state.workload.likelyNonVisual < 0.72) then

				local candidates = getEnabledCandidates()
				local best, bestScore = nil, -math.huge
				for _, candidate in ipairs(candidates) do
					local score = computeFinalRank(candidate)
	...
	if state.workloadTimer >= CONFIG.WorkloadRefreshInterval then
		state.workloadTimer -= CONFIG.WorkloadRefreshInterval
		updateWorkloadProfile()
	end
...
task.spawn(function()
	...
	while gui.Parent do
		...
		appliedLabel.Text = ("%d / %d objects disabled  •  target %d FPS"):format(state.disabledCount, #registry, state.targetFPS)
...
addLog(("CokeBoys-APE started (%s profile, target %d FPS)"):format(IS_MOBILE and "mobile" or "PC", state.targetFPS), "info")
