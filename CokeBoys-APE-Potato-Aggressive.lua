...
local CONFIG = {
	...
	-- Potato Mode only: maximum client-side visual reduction.
	PotatoParticleRateScale = 0,
	PotatoTrailLifetimeScale = 0.01,
...
Workspace.DescendantAdded:Connect(function(inst)
	...
	if state.potatoMode then
		if inst:IsA("ParticleEmitter") then pcall(function() inst.Enabled = false; inst.Rate = 0; inst.TimeScale = 0 end)
		elseif inst:IsA("Trail") then pcall(function() inst.Enabled = false; inst.Lifetime = 0.01 end)
		elseif inst:IsA("Beam") then pcall(function() inst.Enabled = false; inst.Segments = CONFIG.PotatoBeamSegments end)
		elseif inst:IsA("Fire") then pcall(function() inst.Enabled = false; inst.Heat = 0; inst.Size = 0 end)
		elseif inst:IsA("Smoke") then pcall(function() inst.Enabled = false; inst.Opacity = 0; inst.Size = 0 end)
		elseif inst:IsA("Sparkles") then pcall(function() inst.Enabled = false end)
		elseif inst:IsA("BasePart") then pcall(function() inst.Material = Enum.Material.SmoothPlastic; inst.Reflectance = 0; inst.CastShadow = false end)
		elseif inst:IsA("Texture") or inst:IsA("Decal") then pcall(function() inst.Transparency = 1 end)
	end
end)
...
local function potatoCapture()
	...
	for _, inst in ipairs(Workspace:GetDescendants()) do
		...
		elseif inst:IsA("Beam") then
			potatoSnapshot.beams[inst] = {segments = inst.Segments, enabled = inst.Enabled}
		...
		elseif inst:IsA("BasePart") then
			...
			if inst:IsA("MeshPart") then
				pcall(function()
					potatoSnapshot.baseParts[inst].renderFidelity = inst.RenderFidelity
				end)
			end
		elseif inst:IsA("Texture") then
			potatoSnapshot.textures[inst] = inst.Transparency
		elseif inst:IsA("Decal") then
			potatoSnapshot.decals[inst] = inst.Transparency
		end
	end
...
local function potatoApply()
	...
	-- Particles/effects: disable rendering entirely where possible.
	for inst, info in pairs(potatoSnapshot.particles) do
		if inst.Parent then
			pcall(function()
				...
				inst.Rate = 0
				inst.TimeScale = 0
				inst.Lifetime = NumberRange.new(0.03, 0.03)
			end)
		end
	end
	for inst, _ in pairs(potatoSnapshot.trails) do
		if inst.Parent then pcall(function() inst.Enabled = false; inst.Lifetime = 0.01 end) end
	end
	for inst, _ in pairs(potatoSnapshot.beams) do
		if inst.Parent then pcall(function() inst.Enabled = false; inst.Segments = CONFIG.PotatoBeamSegments end) end
	end
	for inst, _ in pairs(potatoSnapshot.fires) do
		if inst.Parent then pcall(function() inst.Enabled = false; inst.Heat = 0; inst.Size = 0 end) end
	end
	for inst, _ in pairs(potatoSnapshot.smokes) do
		if inst.Parent then pcall(function() inst.Enabled = false; inst.Opacity = 0; inst.Size = 0 end) end
	end
	...
	-- Geometry/material simplification: intentionally ugly for maximum FPS.
	...
	for inst, _ in pairs(potatoSnapshot.textures) do
		if inst.Parent then pcall(function() inst.Transparency = 1 end) end
	end
	for inst, _ in pairs(potatoSnapshot.decals) do
		if inst.Parent then pcall(function() inst.Transparency = 1 end) end
	end
...
local function potatoRestore()
	...
	for inst, info in pairs(potatoSnapshot.particles) do
		if inst.Parent then pcall(function()
			inst.Rate = info.rate
			inst.TimeScale = info.timeScale
			inst.Lifetime = info.lifetime
	...
	for inst, info in pairs(potatoSnapshot.beams) do
		if inst.Parent then pcall(function() inst.Segments = info.segments; inst.Enabled = info.enabled end) end
	end
	...
	for inst, info in pairs(potatoSnapshot.baseParts) do
		if inst.Parent then
			...
			if inst:IsA("MeshPart") and info.renderFidelity ~= nil then
				pcall(function() inst.RenderFidelity = info.renderFidelity end)
			end
		end
	end
	for inst, transparency in pairs(potatoSnapshot.textures) do
		if inst.Parent then pcall(function() inst.Transparency = transparency end) end
	end
	for inst, transparency in pairs(potatoSnapshot.decals) do
		if inst.Parent then pcall(function() inst.Transparency = transparency end) end
	end
