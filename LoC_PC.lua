...
local CONFIG = {
    -- PC profile: tuned for high-refresh / uncapped clients.
    SampleWindow = 0.5,
    HistoryLength = 120,
    FrameRingSize = 120,
...
for i = 1, CONFIG.FrameRingSize do
    frameRing[i] = 1 / CONFIG.DefaultTargetFPS
...
local function estimateCost(inst)
    ...
    elseif inst:IsA("Fire") then return KIND_BASE_WEIGHT.Fire * math.clamp((inst.Size or 5) * 5, 1, 120)
...
local function registerInstance(inst)
    ...
    local entry = {
        ...
        contextFPS = 0, contextJitter = 0, lastObservedEnabled = initialEnabled,
        cooldownUntil = 0,
    }
...
local function cachePostEffect(inst)
    if inst:IsA("PostEffect") and postEffects[inst] == nil then
        ...
        if ok then postEffects[inst] = enabled end
    end
end
...
local function resortRegistry()
    for i = #registry, 1, -1 do
        ...
        if not e.inst.Parent then
            registryLookup[e.inst] = nil
            table.remove(registry, i)
        elseif os.clock() - (e.lastCostRefresh or 0) >= CONFIG.RegistryRefreshInterval then
            e.cost = estimateCost(e.inst)
            e.lastCostRefresh = os.clock()
        end
    end
end
    ...
    for _, e in ipairs(registry) do
        if e.inst.Parent then
            ...
            if ok then
                ...
                elseif not e.disabledByUs then
...
local function computeFinalRank(e)
    if not e.inst.Parent or e.originalValue ~= true then return -math.huge end
    local cost = math.max(e.cost, 0)
    if cost < CONFIG.MinUsefulCost and e.sampleCount > 0 then return -math.huge end
    local learned = e.sampleCount > 0 and math.clamp(e.learnedImpact / math.max(cost, 0.01), 0, 2.5) or 0
    local exploration = e.sampleCount == 0 and math.max(cost * 0.16, 1.5)
        or (1 / math.sqrt(e.sampleCount + 1)) * math.max(cost * 0.10, 0.6)
    local confidence = e.confidence * math.max(cost, 1) * 0.20
    local spike = state.diagnosis == "spiky" and e.spikeAssociation * 16 or e.spikeAssociation * 2
    return cost * (1 + learned) + exploration + confidence + spike
end

local function collectCandidates(wantEnabled)
    local list = {}
    local now = os.clock()
    for _, e in ipairs(registry) do
        if e.inst.Parent and e.originalValue == true and now >= (e.cooldownUntil or 0) then
            local ok, enabled = pcall(function() return e.inst.Enabled end)
            if ok then
                if wantEnabled and enabled and not e.disabledByUs then
                    list[#list + 1] = e
                elseif not wantEnabled and e.disabledByUs and not enabled then
                    list[#list + 1] = e
                end
            end
        end
...
local function updateDiagnosis()
    ...
    if state.p95FrameTimeMs > math.max(state.frameTimeMs * 1.65, (1000 / target) * 1.8)
...
local function median(values)
    ...
    table.sort(copy)
    local n = #copy
    if n % 2 == 1 then return copy[(n + 1) * 0.5] end
    return (copy[n * 0.5] + copy[n * 0.5 + 1]) * 0.5
end

local function calculateP95()
    local copy = table.clone(frameRing)
    table.sort(copy)
    local index = math.clamp(math.ceil(#copy * 0.95), 1, #copy)
...
local function contextSignature()
    return {fps = state.smoothedFPS, jitter = state.jitterMs, p95 = state.p95FrameTimeMs, intensity = state.intensity, diagnosis = state.diagnosis}
end
...
local function contextDistance(a, b)
    ...
    return math.clamp(fps * 0.45 + jitter * 0.15 + p95 * 0.20 + intensity * 0.15 + diag * 0.05, 0, 1)
end
...
local function startAction(batch, direction, label)
    ...
    if e.experimentCount >= CONFIG.MaxExperimentsPerObject or now - e.lastTestAt < CONFIG.ExperimentCooldown then return false end
    ...
    e.experimentCount += 1
    pendingAction = {
        e = e, baseline = baseline, baselineSamples = baselineSamples, afterSamples = {},
        direction = direction, startClock = now, beforeContext = contextSignature(),
        label = label, phase = "settle", sampleTimer = 0,
    }
    state.actionState = "settling"
    addLog(("%s %s — causal A/B test started (baseline %.2fms)"):format(direction == "disable" and "Testing OFF" or "Testing ON", entryName(e), baseline), "info")
    return true
end
...
local function updateLearnedImpact(e, measured, contextOK)
    ...
    if contextOK then
        ...
        local denominator = math.max(math.abs(e.impactEMA), CONFIG.NoiseFloorMs * 2)
        local quality = math.clamp(1 - e.impactVariance / denominator, 0, 1)
...
local function resolveAction()
    ...
    if not e or not e.inst.Parent or after <= 0 then
        pendingAction = nil; state.actionState = "idle"; return
    end
...
local function updateExperiment(dt)
    ...
    if v > 0 and v < 250 then action.afterSamples[#action.afterSamples + 1] = v end
...
local function runDecisionCycle()
    ...
    local desiredCount = math.floor(#registry * math.max(state.intensity - CONFIG.MinDisableIntensity, 0) + 0.5)
    ...
    if desiredCount > currentDisabled then
        ...
        for _, candidate in ipairs(candidates) do if startAction({candidate}, "disable", "autonomous causal test") then break end end
    elseif desiredCount < currentDisabled and state.intensity <= CONFIG.RecoveryHysteresis then
        ...
        for _, candidate in ipairs(candidates) do if startAction({candidate}, "enable", "least-useful restore test") then break end end
    end
end
...
local function forceRestoreAll()
    ...
    if postEffectsDisabledByUs then
        for inst, wasEnabled in pairs(postEffects) do if inst.Parent then pcall(function() inst.Enabled = wasEnabled end) end end
        postEffectsDisabledByUs = false
    end
    if shadowsDisabledByUs then pcall(function() Lighting.GlobalShadows = originalGlobalShadows end); shadowsDisabledByUs = false end
    if streamingReducedByUs then pcall(function() Workspace.StreamingTargetRadius = originalStreamingRadius end); streamingReducedByUs = false end
    state.intensity = 0
end

local function applyGlobalEffects(intensity)
    if intensity >= 0.5 and not postEffectsDisabledByUs then
        for inst in pairs(postEffects) do if inst.Parent then pcall(function() inst.Enabled = false end) end end
    ...
    if intensity >= 0.6 and originalStreamingRadius > CONFIG.StreamingRadiusFloor then
        ...
        local target = math.floor(originalStreamingRadius - t * (originalStreamingRadius - CONFIG.StreamingRadiusFloor))
        pcall(function() Workspace.StreamingTargetRadius = math.max(CONFIG.StreamingRadiusFloor, target) end)
...
local function stepController(dt)
    ...
    local target, fps = math.max(state.targetFPS, 1), state.smoothedFPS
    local ratio = fps / target
    if ratio < 0.985 then
        local err = math.clamp((target - fps) / target, 0, 1)
        local severity = state.diagnosis == "sustained" and 1.20 or state.diagnosis == "spiky" and 0.55 or 0.80
        state.intensity = math.clamp(state.intensity + CONFIG.AttackRate * err * severity * dt, 0, 1)
    elseif ratio > CONFIG.RecoveryMargin and state.jitterMs < CONFIG.JitterSpikyThreshold then
        local headroom = math.clamp(ratio - CONFIG.RecoveryMargin, 0, 1)
        state.intensity = math.clamp(state.intensity - CONFIG.DecayRate * (0.55 + headroom) * dt, 0, 1)
    end
end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if dt > mean * CONFIG.SpikeMultiplier and dt > CONFIG.SpikeMinDelta then
        ...
        if now - state.lastSpikeAt >= CONFIG.SpikeCooldown then
            ...
            for _, e in ipairs(collectCandidates(true)) do if e.cost > 10 then e.spikeAssociation += 1 end end
            addLog(("Spike detected (%.1fms frame)"):format(dt * 1000), "warn")
...
local function computeHealthScore()
    ...
    local p95Score = math.clamp(100 - math.max(0, state.p95FrameTimeMs - (1000 / math.max(state.targetFPS, 1))) * 18, 0, 100)
    local pingScore = state.ping <= 50 and 100 or state.ping <= 100 and 80 or state.ping <= 150 and 60 or state.ping <= 250 and 40 or 20
    return math.floor(fpsScore * 0.45 + jitterScore * 0.20 + p95Score * 0.20 + pingScore * 0.15)
end
...
sectionLabel(settingsPage, "MANUAL OVERRIDE", 5)
local restoreRow = card(settingsPage, UDim2.new(1, 0, 0, 44), 6)
