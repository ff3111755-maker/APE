...
local function potatoCaptureVisual(inst)
	...
	if inst:IsA("BasePart") and not potatoSnapshot.baseParts[inst] then
		...
		if reflectance > CONFIG.PotatoReflectanceCutoff or (CONFIG.PotatoDisableCastShadow and castShadow) then
			if (function() local n=0 for _ in pairs(potatoSnapshot.baseParts) do n = n + 1 end return n end)() < CONFIG.PotatoMaxTrackedVisuals then
...
local function potatoApplyVisual(inst)
	...
	if inst:IsA("BasePart") then
		...
		if info then pcall(function()
			if info.reflectance > CONFIG.PotatoReflectanceCutoff then
				inst.Reflectance = 0
			end
...
local function potatoCaptureVisuals()
	...
	for _, inst in ipairs(Workspace:GetDescendants()) do
		if tracked >= CONFIG.PotatoMaxTrackedVisuals then
			break
		end
		potatoCaptureVisual(inst)
		tracked = tracked + 1
...
local function potatoCapture()
	if potatoSnapshot.captured then
		return
	end
	clearPotatoSnapshot()
...
potatoApplyInstance = function(inst)
	if not potatoSnapshot.captured or not inst or not inst.Parent then
		return
	end
	potatoCaptureInstance(inst)
...
local function potatoRestore()
	if not potatoSnapshot.captured then
		return
	end
	...
	pcall(function()
		if potatoSnapshot.lighting.globalShadows ~= nil then
			Lighting.GlobalShadows = potatoSnapshot.lighting.globalShadows
		end
		if potatoSnapshot.lighting.environmentDiffuseScale ~= nil then
			Lighting.EnvironmentDiffuseScale = potatoSnapshot.lighting.environmentDiffuseScale
		end
		if potatoSnapshot.lighting.environmentSpecularScale ~= nil then
			Lighting.EnvironmentSpecularScale = potatoSnapshot.lighting.environmentSpecularScale
		end
		if potatoSnapshot.lighting.brightness ~= nil then
			Lighting.Brightness = potatoSnapshot.lighting.brightness
		end
	end)
...
local function rebuildCandidateCaches()
	if not candidateCacheDirty then
		return
	end
...
local function startAction(batch, direction, label)
	...
	if e.experimentCount >= CONFIG.MaxExperimentsPerObject then
		return false
	end
	if os.clock() - e.lastTestAt < CONFIG.ExperimentCooldown then
		return false
	end
	...
	local oldValue
	if not pcall(function() oldValue = e.inst.Enabled end) then
		return false
	end
	...
	e.experimentCount = e.experimentCount + 1
	pendingAction = {
		e = e,
		baseline = baseline,
		afterSamples = {},
...
local function resolveAction()
	...
	if action.direction == "disable" then
		if meaningful then
			e.successCount = e.successCount + 1
			...
			state.session.optimizationEvents = state.session.optimizationEvents + 1
			onEngineEvent("kept", e, delta)
		else
			e.failCount = e.failCount + 1
...
local function updateExperiment(dt)
	...
	action.sampleTimer = action.sampleTimer + dt
...
local function currentDisabledCost()
	local total = 0
	for _, e in ipairs(collectCandidates(false)) do total = total + e.cost end
	return total
end

local function totalEligibleCost()
	local total = 0
	for _, e in ipairs(collectCandidates(true)) do total = total + e.cost end
	for _, e in ipairs(collectCandidates(false)) do total = total + e.cost end
	return total
end
...
local function runDecisionCycle()
	...
	local current = 0
	for _, e in ipairs(disabled) do current = current + e.cost end
	if budget > current and #enabled > 0 then
		...
		for _, candidate in ipairs(enabled) do
			if startAction({candidate}, "disable", "performance test") then
				break
			end
		end
	elseif budget + 1 < current and #disabled > 0 then
		...
		for _, candidate in ipairs(disabled) do
			if startAction({candidate}, "enable", "least-useful restore test") then
				break
			end
		end
	end
...
local function applyGlobalEffects(intensity)
	...
	if intensity >= 0.6 and originalStreamingRadius > CONFIG.StreamingRadiusFloor then
		...
		if math.abs(target - lastStreamingTarget) >= 8 then
			...
			if ok then
				lastStreamingTarget = target; streamingReducedByUs = true
			end
		end
...
local function stepController(dt)
	if not state.autoOptimize or state.warmupTimer < CONFIG.WarmupTime then
		return
	end
...
local function detectSpike(dt, mean)
	if state.warmupTimer < CONFIG.WarmupTime or frameSampleCount < 12 then
		return false
	end
...
local function pushRenderTime(dt)
	...
	if renderSampleCount < #renderRing then renderSampleCount = renderSampleCount + 1 end
...
local function adaptiveEvaluate(dt)
	if not CONFIG.AdaptiveEnabled or not state.autoOptimize or state.warmupTimer < CONFIG.WarmupTime then
		return
	end
	state.adaptiveTimer = state.adaptiveTimer + dt
	if state.adaptiveTimer < CONFIG.AdaptiveTick then
		return
	end
	...
	if bad then
		state.adaptiveBadTimer = state.adaptiveBadTimer + elapsed
		state.adaptiveGoodTimer = 0
	else
		state.adaptiveBadTimer = math.max(0, state.adaptiveBadTimer - elapsed * 0.5)
	end
	if good then
		state.adaptiveGoodTimer = state.adaptiveGoodTimer + elapsed
	else
...
local function potatoEnforce(dt)
	...
	potatoEnforcerTimer = potatoEnforcerTimer + dt
	if potatoEnforcerTimer < CONFIG.PotatoSweepInterval then
		return
	end
...
RunService.Heartbeat:Connect(function(dt)
	...
	state.warmupTimer = state.warmupTimer + dt
	stepController(dt)
	updateExperiment(dt)
	potatoEnforce(dt)
	adaptiveEvaluate(dt)
	...
	state.session.timeInBand[band] = state.session.timeInBand[band] + dt
	state.sampleTimer = state.sampleTimer + dt
	state.resortTimer = state.resortTimer + dt
	state.spikeDecayTimer = state.spikeDecayTimer + dt
	state.decisionTimer = state.decisionTimer + dt
	state.maintenanceTimer = state.maintenanceTimer + dt
	state.costTimer = state.costTimer + dt

	state.spikeWindowTimer = state.spikeWindowTimer + dt
	if state.spikeWindowTimer >= CONFIG.SpikeWindow then
		state.spikeWindowTimer = state.spikeWindowTimer - CONFIG.SpikeWindow
		state.spikeCountWindow = 0
	end
	if detectSpike(dt, mean) then
		state.spikeCountWindow = state.spikeCountWindow + 1
		local now = os.clock()
		if now - state.lastSpikeAt >= CONFIG.SpikeCooldown then
			...
			for _, e in ipairs(collectCandidates(true)) do if e.cost > 15 then e.spikeAssociation = e.spikeAssociation + 1 end end
	...
	if state.sampleTimer >= CONFIG.SampleWindow then
		state.sampleTimer = state.sampleTimer - CONFIG.SampleWindow
		...
		s.fpsMin = math.min(s.fpsMin, state.smoothedFPS); s.fpsMax = math.max(s.fpsMax, state.smoothedFPS); s.fpsSum = s.fpsSum + state.smoothedFPS; s.fpsCount = s.fpsCount + 1
		s.pingMin = math.min(s.pingMin, state.ping); s.pingMax = math.max(s.pingMax, state.ping); s.pingSum = s.pingSum + state.ping; s.pingCount = s.pingCount + 1
	...
	if state.maintenanceTimer >= CONFIG.RegistryMaintenanceInterval then
		state.maintenanceTimer = state.maintenanceTimer - CONFIG.RegistryMaintenanceInterval
		resortRegistry()
		syncExternalState()
	end
	if state.costTimer >= CONFIG.CostRefreshInterval then
		state.costTimer = state.costTimer - CONFIG.CostRefreshInterval
		refreshCosts()
	end
	if state.spikeDecayTimer >= CONFIG.SpikeDecayInterval then
		state.spikeDecayTimer = state.spikeDecayTimer - CONFIG.SpikeDecayInterval
		for _, e in ipairs(registry) do e.spikeAssociation = e.spikeAssociation * CONFIG.SpikeDecayFactor end
		invalidateCandidates()
	end
	if state.autoOptimize and state.actionState == "idle" and state.decisionTimer >= CONFIG.DecisionInterval then
		state.decisionTimer = state.decisionTimer - CONFIG.DecisionInterval
		runDecisionCycle()
	end
end)
...
local function maxUpdatePressure()
	...
	for _, e in ipairs(registry) do
		if e and maxAlive(e.inst) then
			...
			visual = visual + cost
			if e.disabledByUs then disabled = disabled + cost end
			if count >= MAX.MaxTrackedObjects then
				break
			end
		end
	end
...
local function maxApplyIntermediate(inst, tier)
	if not maxAlive(inst) or tier <= 0 then
		return
	end
...
local function maxSweep()
	...
	for _ = 1, budget do
		...
		if entry and maxAlive(entry.inst) then
			...
			if ok then maxState.objectsOptimized = maxState.objectsOptimized + 1 end
...
local function maxDynamic(inst)
	if not MAX.Enabled or not inst then
		return
	end
	maxState.dynamicEvents = maxState.dynamicEvents + 1
	maxState.objectsSeen = maxState.objectsSeen + 1
...
local function maxSetTier(tier, reason)
	if not maxAllowed() or maxManualPotato() then
		return
	end
...
local function maxEmergency()
	...
	if not maxCooldown("emergency", MAX.SpikeCooldown) then
		return
	end
...
local function maxRestoreGuard()
	...
	if state.adaptiveTier >= 3 then
		return false
	end
	if not maxAllowed() then
		return false
	end
	if maxState.pressure > 0.15 then
		return false
	end
	return true
end
...
local function maxQualityRecovery()
	if not maxRestoreGuard() then
		return
	end
	if state.actionState ~= "idle" then
		return
	end
	if not maxCooldown("quality-recovery", MAX.RecoveryDelay) then
		return
	end
...
local function maxLearn(entry, result, delta)
	...
	if total > MAX.MaxLearningRecords then
		...
		for k in pairs(maxLearning) do
			maxLearning[k] = nil
			removed = removed + 1
			if removed >= math.floor(MAX.MaxLearningRecords * 0.10) then
				break
			end
		end
	end
...
maxConnections[#maxConnections + 1] = RunService.Heartbeat:Connect(function(dt)
	...
	maxTimer = maxTimer + math.max(dt, 0)

	if maxTimer >= MAX.ControllerInterval then
		maxTimer = maxTimer - MAX.ControllerInterval
...
local function v7IsCritical(inst)
	...
	if V7.DoNotTouchPlayerGui and inst:IsDescendantOf(playerGui) then
		return true
	end
	if V7.DoNotTouchCamera and Workspace.CurrentCamera and inst:IsDescendantOf(Workspace.CurrentCamera) then
		return true
	end
	...
	if V7.DoNotTouchAnimations and inst:IsA("Animator") then
		return true
	end
	...
	if inst:IsA("BindableEvent") or inst:IsA("BindableFunction") then
		return true
	end
	return false
end
...
local function v7ProtectedReason(inst)
	...
	if v7IsCritical(inst) then
		return "engine-critical"
	end
	if v7IsCharacterPart(inst) then
		return "character"
	end
...
local function v7Snapshot(tab, inst, data)
	if v7SnapshotCount >= V7.MaxSnapshotEntries then
		return false
	end
	if tab[inst] ~= nil then
		return false
	end
	tab[inst] = data
	v7State.snapshotCount = v7State.snapshotCount + 1
	return true
end
...
local function v7Cost(inst)
	...
	if inst:IsA("PostEffect") then
		...
		if inst:IsA("SunRaysEffect") then
			return 40
		end
		return 30
	end
...
local function v7Register(inst)
	if not inst or v7Tracked[inst] then
		return
	end
	v7Tracked[inst] = true
	v7Protect(inst)
	...
	if kind == "particle" or kind == "trail" or kind == "beam" or kind == "fire"
		...
		if kind == "particle" or kind == "trail" or kind == "beam"
			...
			v7State.effectCount = v7State.effectCount + 1
		elseif kind == "light" then
			v7State.lightCount = v7State.lightCount + 1
		elseif kind == "post" or kind == "atmosphere" or kind == "clouds" then
			v7State.postCount = v7State.postCount + 1
		elseif kind == "part" then
			v7State.partCount = v7State.partCount + 1
		end
	end
end
...
local function v7Seed()
	local count = 0
	for _, inst in ipairs(Workspace:GetDescendants()) do
		if count >= V7.MaxTrackedInstances then
			break
		end
		v7Register(inst)
		count = count + 1
...
local function v7RememberParticle(inst)
	if v7Snapshot.particles[inst] then
		return
	end
...
local function v7RememberTrail(inst)
	if v7Snapshot.trails[inst] then
		return
	end
...
local function v7RememberSmoke(inst)
	if v7Snapshot.smokes[inst] then
		return
	end
...
local function v7RememberSparkles(inst)
	if v7Snapshot.sparkles[inst] then
		return
	end
...
local function v7RememberHighlight(inst)
	if v7Snapshot.highlights[inst] then
		return
	end
...
local function v7RememberAtmosphere(inst)
	if v7Snapshot.atmosphere[inst] then
		return
	end
...
local function v7RememberClouds(inst)
	if v7Snapshot.clouds[inst] then
		return
	end
...
local function v7RememberLight(inst)
	if v7Snapshot.lights[inst] then
		return
	end
...
local function v7RememberPart(inst)
	if v7Snapshot.parts[inst] then
		return
	end
	local reflectance = v7SafeGet(inst, "Reflectance", 0)
	local castShadow = v7SafeGet(inst, "CastShadow", true)
	if reflectance <= V7.ReflectancePerformance and not castShadow then
		return
	end
	if v7State.snapshotCount >= V7.MaxSnapshotEntries then
		return
	end
...
local function v7Remember(inst)
	if not v7IsAlive(inst) or v7ShouldSkip(inst) then
		return false
	end
	local kind = v7EffectType(inst)
	if not kind then
...
local function v7ApplyPart(inst, profile)
	if not V7.SpecularReduction then
		return false
	end
	if v7IsCharacterPart(inst) then
		return false
...
local function v7Apply(inst, profile)
	...
	if changed then
		v7State.objectsChanged = v7State.objectsChanged + 1
	end
	v7State.objectsTouched = v7State.objectsTouched + 1
	return changed
end
...
local function v7RestoreAll()
	...
	pcall(function()
		if v7Snapshot.lighting.GlobalShadows ~= nil then
			Lighting.GlobalShadows = v7Snapshot.lighting.GlobalShadows
		end
		if v7Snapshot.lighting.EnvironmentDiffuseScale ~= nil then
			Lighting.EnvironmentDiffuseScale = v7Snapshot.lighting.EnvironmentDiffuseScale
		end
		if v7Snapshot.lighting.EnvironmentSpecularScale ~= nil then
			Lighting.EnvironmentSpecularScale = v7Snapshot.lighting.EnvironmentSpecularScale
		end
	end)
...
local function v7Measure()
	...
	for i = 1, #v7State.workspaceList do
		...
		if i >= V7.MaxTrackedInstances then
			break
		end
	end
...
local function v7DesiredProfile()
	...
	if state.potatoMode and state.adaptiveTier >= 3 then
		return "potato"
	end
...
local function v7SetProfile(profile, reason)
	if profile == v7State.currentProfile then
		return
	end
...
local function v7OnAdded(inst)
	v7State.dynamicAdded = v7State.dynamicAdded + 1
	v7Register(inst)
...
local function v7PotatoEnable()
	...
	while lightingCursor <= #v7State.lightingList and lightingCursor <= 600 do
		...
		lightingCursor = lightingCursor + 1
	end
...
local function v7Controller(dt)
	...
	v7State.metricClock = v7State.metricClock + dt
	v7State.profileClock = v7State.profileClock + dt
	v7State.reconcileClock = v7State.reconcileClock + dt
	v7State.spikeClock = v7State.spikeClock + dt

	if v7State.metricClock >= V7.MetricInterval then
		v7State.metricClock = v7State.metricClock - V7.MetricInterval
		v7Measure()
	end

	if v7State.reconcileClock >= V7.ReconcileInterval then
		v7State.reconcileClock = v7State.reconcileClock - V7.ReconcileInterval
		v7Reconcile()
	end

	if v7State.profileClock >= V7.ProfileInterval then
		v7State.profileClock = v7State.profileClock - V7.ProfileInterval
...
local function v7BurstRegister()
	...
	v7Burst.created = v7Burst.created + 1
...
local function v7OwnershipCheck()
	local now = v7Now()
	if now - v7LastOwnershipCheck < 3 then
		return
	end
...
local function v8Distance(inst)
	...
	if not camPos or not inst then
		return math.huge
	end
...
local function v8DistanceWeight(distance)
	if distance <= V8.NearDistance then
		return V8.NearWeight, "near"
	end
	if distance <= V8.MidDistance then
		return V8.MidWeight, "mid"
	end
	if distance <= V8.FarDistance then
		return V8.FarWeight, "far"
	end
...
local function v8BaseCost(inst, kind)
	...
	if kind == "sparkles" then
		return 16 * V8.SparklesMultiplier
	end
	if kind == "highlight" then
		return 24 * V8.HighlightMultiplier
	end
	if kind == "light" then
	...
	if kind == "post" then
		return 45 * V8.PostMultiplier
	end
	if kind == "atmosphere" then
		return 75 * V8.PostMultiplier
	end
	if kind == "clouds" then
		return 55 * V8.PostMultiplier
	end
	return 0
end

local function v8IsPlayerCharacter(inst)
	if not V8.ProtectAllPlayerCharacters then
		return false
	end
	local p = inst
	for _ = 1, 10 do
...
local function v8IsProtected(inst)
	...
	if v8Protected[inst] then
		return true
	end
	if V8.ProtectUI and inst:IsDescendantOf(playerGui) then
		return true
	end
	if V8.ProtectLocalCharacter and player.Character and inst:IsDescendantOf(player.Character) then
		return true
	end
	if V8.ProtectCameraRegion and Workspace.CurrentCamera and inst:IsDescendantOf(Workspace.CurrentCamera) then
		return true
	end
	if v8IsPlayerCharacter(inst) then
		return true
	end
	if inst:IsA("Humanoid") or inst:IsA("Animator") or inst:IsA("AnimationController") then
		return true
	end
	return false
end
...
local function v8CreateRecord(inst)
	if not v8Alive(inst) or v8IsProtected(inst) then
		if v8IsProtected(inst) then v8.totalProtected = v8.totalProtected + 1 end
		return nil
	end
	...
	if kind == "part" and v7IsCharacterPart(inst) then
		return nil
	end
	...
	v8.totalScored = v8.totalScored + 1

	pcall(function()
		inst.Destroying:Connect(function()
			v8.records[inst] = nil
			v8.totalExpired = v8.totalExpired + 1
		end)
	end)
...
local function v8Score(record)
	if not record or not v8Alive(record.inst) then
		return 0
	end
	...
	if band == "near" then v8.near = v8.near + 1
	elseif band == "mid" then v8.mid = v8.mid + 1
	elseif band == "far" then v8.far = v8.far + 1
	else v8.veryFar = v8.veryFar + 1 end
...
local function v8RefreshDistances()
	...
	for i = 1, #v8.recordList do
		...
		if record and v8Alive(record.inst) then
			...
			v8.effectCost = v8.effectCost + record.baseCost
			v8.weightedCost = v8.weightedCost + score
		end
	end
end
...
local function v8ApplyPartAntiPlastic(inst, profile)
	...
	if v8IsProtected(inst) then
		return false
	end
	if not V8.PreserveMaterials then
		return false
	end
...
local function v8ApplyRecord(record, profile)
	...
	if v8IsProtected(record.inst) then
		...
		record.skipped = record.skipped + 1
		v8.totalSkipped = v8.totalSkipped + 1
		return false
	end
	...
	v8.totalProcessed = v8.totalProcessed + 1
	if changed then
		record.changed = record.changed + 1
		v8.totalChanged = v8.totalChanged + 1
	end
	return changed
end

local function v8Compact()
	if #v8.recordList < 200 then
		return
	end
	local write = 1
	...
	if v8.cursor > #v8.recordList then
		v8.cursor = 1
	end
end
...
local function v8SpikeRegister()
	...
	v8.burstCreated = v8.burstCreated + 1
...
local function v8OnAdded(inst)
	...
	v8.dynamicAdded = v8.dynamicAdded + 1
	v8CreateRecord(inst)
...
local function v8Transition(profile)
	if profile == v8.lastProfile then
		return
	end
...
local function v8Controller(dt)
	...
	v8.rebuildClock = v8.rebuildClock + dt
	v8.distanceClock = v8.distanceClock + dt
	v8.profileClock = v8.profileClock + dt
	v8.protectedClock = v8.protectedClock + dt

	if v8.rebuildClock >= V8.RebuildSeconds then
		v8.rebuildClock = v8.rebuildClock - V8.RebuildSeconds
		v8Rebuild()
		v8Compact()
	end

	if v8.distanceClock >= V8.DistanceRefreshSeconds then
		v8.distanceClock = v8.distanceClock - V8.DistanceRefreshSeconds
		v8RefreshDistances()
		v8SortByScore()
	end

	if v8.profileClock >= V8.ProfileRefreshSeconds then
		v8.profileClock = v8.profileClock - V8.ProfileRefreshSeconds
	...
	for i = 1, #v7State.lightingList do
		...
		if v8Alive(inst) then
			...
			if kind == "light" or kind == "post" or kind == "atmosphere" or kind == "clouds" then
				cost = cost + v8BaseCost(inst, kind)
				count = count + 1
			end
		end
		if count >= 2500 then
...
local function v8SelfBudget(dt)
	...
	if v8SelfMonitor.ema > budget then
		v8SelfMonitor.overBudget = v8SelfMonitor.overBudget + 1
		v8SelfMonitor.underBudget = 0
	else
		v8SelfMonitor.underBudget = v8SelfMonitor.underBudget + 1
	...
	for i = 1, math.min(#v8.recordList, 12000) do
		...
		if r and v8Alive(r.inst) then
			...
			if kind ~= "part" then
				...
				if enabled then activeEffects = activeEffects + 1 end
			end
		end
	end
...
local function v8CanRecover()
	...
	if not state.autoOptimize then
		return false
	end
	if v7State.pressure > 0.16 then
		return false
	end
	if math.max(state.jitterMs, state.renderJitterMs) > CONFIG.JitterSpikyThreshold * 0.60 then
		return false
	end
	return true
end
...
local function v8Recovery(dt)
	...
	v8RecoveryClock = v8RecoveryClock + dt
	if v8RecoveryClock < V8.StableRecoverySeconds then
		return
	end
	v8RecoveryClock = 0
...
v8Connections[#v8Connections + 1] = RunService.Heartbeat:Connect(function(dt)
	...
	v8MetricClock = v8MetricClock + dt
	if v8MetricClock >= 1.0 then
		v8MetricClock = v8MetricClock - 1.0
		pcall(v8Metrics)
	end
...
local function v9IsProtected(inst)
	...
	if Workspace.CurrentCamera and inst:IsDescendantOf(Workspace.CurrentCamera) then
		return true
	end
	if inst:IsDescendantOf(playerGui) then
		return true
	end
	if inst:IsA("Script") or inst:IsA("LocalScript") or inst:IsA("ModuleScript") then
		return true
...
local function v9ProfileName()
	...
	local p = v7State.pressure
	if p >= V9.ExtremePressure then
		return "extreme"
	end
	if p >= V9.PotatoPressure then
		return "potato"
	end
	if p >= V9.PerformancePressure then
		return "performance"
	end
	if p >= V9.BalancedPressure then
		return "balanced"
	end
	return "quality"
end
...
local function v9TransitionAllowed(desired)
	...
	if newRank > oldRank then
		...
		if desired == "performance" then
			hold = V9.EscalatePerformance
		end
		if desired == "potato" then
			hold = V9.EscalatePotato
		end
		if desired == "extreme" then
			hold = V9.EscalateExtreme
		end
		return elapsed >= hold
	end
...
local function v9Transition(profile, reason)
	...
	if not v9TransitionAllowed(profile) then
		return false
	end
	...
	v9State.profileChanges = v9State.profileChanges + 1
...
local function v9CaptureIfNeeded(inst)
	if not v9Alive(inst) or v9IsProtected(inst) then
		return false
	end
	if not v9SnapshotHealthy(inst) then
		v9State.integrityFailures = v9State.integrityFailures + 1
	end
	...
	v9State.integrityChecks = v9State.integrityChecks + 1
	return true
end
...
local function v9RepairOne(inst, profile)
	...
	if changed then
		v9State.integrityRepairs = v9State.integrityRepairs + 1
	end
	return changed
end
...
local function v9IntegrityPass()
	...
	for i = 1, #v8.recordList do
		if checks >= V9.MaxIntegrityChecks then
			break
		end
		local record = v8.recordList[i]
	...
	v9State.integrityChecks = v9State.integrityChecks + checks
	v9State.integrityRepairs = v9State.integrityRepairs + repairs
end
...
local function v9FindSuspicious()
	...
	for i = 1, #v8.recordList do
		if count >= V9.MaxDiagnosticsObjects then
			break
		end
...
local function v9LogDiagnostics()
	...
	v9State.diagnosticPasses = v9State.diagnosticPasses + 1
...
local function v9Recover()
	...
	if v7State.pressure <= V9.QualityPressure
		...
		v9State.stableClock = v9State.stableClock + V9.RecoveryInterval
...
local function v9Rebuild()
	v9State.rebuildPasses = v9State.rebuildPasses + 1

	local added = 0
	for _, inst in ipairs(Workspace:GetDescendants()) do
		if added >= V9.MaxRebuildObjects then
			break
		end
	...
	added = 0
	for _, inst in ipairs(Lighting:GetDescendants()) do
		if added >= math.floor(V9.MaxRebuildObjects * 0.35) then
			break
		end
...
local function v9TrailPolicy(inst, profile)
	if not inst or not inst:IsA("Trail") then
		return false
	end
	if v9IsProtected(inst) then
		return false
...
local function v9BeamPolicy(inst, profile)
	if not inst or not inst:IsA("Beam") then
		return false
	end
	if v9IsProtected(inst) then
		return false
...
local function v9FirePolicy(inst, profile)
	if not inst or not inst:IsA("Fire") then
		return false
	end
	if v9IsProtected(inst) then
		return false
...
local function v9SmokePolicy(inst, profile)
	if not inst or not inst:IsA("Smoke") then
		return false
	end
	if v9IsProtected(inst) then
		return false
...
local function v9SparklePolicy(inst, profile)
	if not inst or not inst:IsA("Sparkles") then
		return false
	end
	if v9IsProtected(inst) then
		return false
...
local function v9HighlightPolicy(inst, profile)
	if not inst or not inst:IsA("Highlight") then
		return false
	end
	if v9IsProtected(inst) then
		return false
...
local function v9PartPolicy(inst, profile)
	...
	if not V9.PreserveMaterialIdentity then
		return false
	end
...
local function v9ApplySpecific(inst, profile)
	local kind = v9Type(inst)
	if kind == "particle" then
		return v9ParticlePolicy(inst, profile)
	end
	if kind == "trail" then
		return v9TrailPolicy(inst, profile)
	end
	if kind == "beam" then
		return v9BeamPolicy(inst, profile)
	end
	if kind == "fire" then
		return v9FirePolicy(inst, profile)
	end
	if kind == "smoke" then
		return v9SmokePolicy(inst, profile)
	end
	if kind == "sparkles" then
		return v9SparklePolicy(inst, profile)
	end
	if kind == "highlight" then
		return v9HighlightPolicy(inst, profile)
	end
	if kind == "light" then
		return v9LightPolicy(inst, profile)
	end
	if kind == "part" then
		return v9PartPolicy(inst, profile)
	end
	if kind == "post" then
		return v7ApplyPost(inst, profile)
	end
	if kind == "atmosphere" then
		return v7ApplyAtmosphere(inst, profile)
	end
	if kind == "clouds" then
		return v7ApplyClouds(inst, profile)
	end
	return false
end
...
local function v9ProcessProfileSlice()
	...
	if not profile or profile == "quality" then
		return
	end
	...
	while done < budget and count > 0 do
		...
		if record and v9Alive(record.inst) then
			if not v9IsProtected(record.inst) then
				...
				v9State.protected = v9State.protected + 1
			end
		end
	end
end
...
v9Connections[#v9Connections + 1] = RunService.Heartbeat:Connect(function(dt)
	if not V9.Enabled or dt <= 0 then
		return
	end

	v9TickClock = v9TickClock + dt
	v9State.integrityClock = v9State.integrityClock + dt
	v9State.diagnosticsClock = v9State.diagnosticsClock + dt
	v9State.rebuildClock = v9State.rebuildClock + dt
	v9State.recoveryClock = v9State.recoveryClock + dt
...
local function refreshLogUI()
	if #state.log == lastLogCount then
		return
	end
...
task.spawn(function()
	...
	while gui.Parent do
		...
		for _, e in ipairs(registry) do if e.disabledByUs then disabledCount = disabledCount + 1 end end
