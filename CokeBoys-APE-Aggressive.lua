...
local CONFIG = {
	...
	PotatoParticleRateScale = 0,
	PotatoTrailLifetimeScale = 0.03,
...
Workspace.DescendantAdded:Connect(function(inst)
	...
	if state.potatoMode then
		pcall(function()
			if inst:IsA("ParticleEmitter") then inst.Enabled=false; inst.Rate=0; inst.TimeScale=0
			elseif inst:IsA("Trail") then inst.Enabled=false
			elseif inst:IsA("Beam") then inst.Enabled=false; inst.Segments=CONFIG.PotatoBeamSegments
			elseif inst:IsA("Fire") then inst.Enabled=false; inst.Heat=0; inst.Size=0
			elseif inst:IsA("Smoke") then inst.Enabled=false; inst.Opacity=0; inst.Size=0
			elseif inst:IsA("Sparkles") then inst.Enabled=false
			elseif inst:IsA("Decal") or inst:IsA("Texture") then inst.Transparency=1
			elseif inst:IsA("PointLight") or inst:IsA("SpotLight") or inst:IsA("SurfaceLight") then inst.Enabled=false
			elseif inst:IsA("BasePart") then inst.Material=Enum.Material.SmoothPlastic; inst.Reflectance=0; inst.CastShadow=false
			end
			if inst:IsA("MeshPart") then inst.RenderFidelity=Enum.RenderFidelity.Performance end
		end)
	end
end)
...
local function potatoCapture()
	...
	pcall(function() potatoSnapshot.lightingTechnology = Lighting.Technology end)
	...
	for _, inst in ipairs(Workspace:GetDescendants()) do
		if inst:IsA("ParticleEmitter") then potatoSnapshot.particles[inst] = {rate=inst.Rate,timeScale=inst.TimeScale,lifetime=inst.Lifetime}
		elseif inst:IsA("Trail") then potatoSnapshot.trails[inst] = {lifetime=inst.Lifetime,enabled=inst.Enabled}
		elseif inst:IsA("Beam") then potatoSnapshot.beams[inst] = {segments=inst.Segments,enabled=inst.Enabled}
		elseif inst:IsA("Fire") then potatoSnapshot.fires[inst] = {heat=inst.Heat,size=inst.Size,enabled=inst.Enabled}
		elseif inst:IsA("Smoke") then potatoSnapshot.smokes[inst] = {opacity=inst.Opacity,size=inst.Size,enabled=inst.Enabled}
		elseif inst:IsA("Sparkles") then potatoSnapshot.sparkles[inst] = {enabled=inst.Enabled}
		elseif inst:IsA("BasePart") then
			potatoSnapshot.baseParts[inst] = {material=inst.Material,reflectance=inst.Reflectance,castShadow=inst.CastShadow}
			if inst:IsA("MeshPart") then pcall(function() potatoSnapshot.meshRenderFidelity[inst]=inst.RenderFidelity end) end
		elseif inst:IsA("Decal") or inst:IsA("Texture") then potatoSnapshot.textures[inst] = {transparency=inst.Transparency}
		elseif inst:IsA("PointLight") or inst:IsA("SpotLight") or inst:IsA("SurfaceLight") then potatoSnapshot.lights[inst] = {enabled=inst.Enabled}
		end
	end
	for _, inst in ipairs(Lighting:GetChildren()) do if inst:IsA("PostEffect") then potatoSnapshot.postEffects[inst]=inst.Enabled end end
end

local function potatoApply()
	potatoCapture()
	for inst,_ in pairs(potatoSnapshot.particles) do if inst.Parent then pcall(function() inst.Enabled=false; inst.Rate=0; inst.TimeScale=0 end) end end
	for inst,info in pairs(potatoSnapshot.trails) do if inst.Parent then pcall(function() inst.Enabled=false; inst.Lifetime=math.max(0.03,info.lifetime*CONFIG.PotatoTrailLifetimeScale) end) end end
	for inst,_ in pairs(potatoSnapshot.beams) do if inst.Parent then pcall(function() inst.Enabled=false; inst.Segments=CONFIG.PotatoBeamSegments end) end end
	for inst,_ in pairs(potatoSnapshot.fires) do if inst.Parent then pcall(function() inst.Enabled=false; inst.Heat=0; inst.Size=0 end) end end
	for inst,_ in pairs(potatoSnapshot.smokes) do if inst.Parent then pcall(function() inst.Enabled=false; inst.Opacity=0; inst.Size=0 end) end end
	for inst,_ in pairs(potatoSnapshot.sparkles) do if inst.Parent then pcall(function() inst.Enabled=false end) end end
	for inst,_ in pairs(potatoSnapshot.postEffects) do if inst.Parent then pcall(function() inst.Enabled=false end) end end
	for inst,_ in pairs(potatoSnapshot.lights) do if inst.Parent then pcall(function() inst.Enabled=false end) end end
	for inst,_ in pairs(potatoSnapshot.textures) do if inst.Parent then pcall(function() inst.Transparency=1 end) end end
	for inst,_ in pairs(potatoSnapshot.baseParts) do if inst.Parent then pcall(function() inst.Material=Enum.Material.SmoothPlastic; inst.Reflectance=0; inst.CastShadow=false end) end end
	for inst,_ in pairs(potatoSnapshot.meshRenderFidelity) do if inst.Parent then pcall(function() inst.RenderFidelity=Enum.RenderFidelity.Performance end) end end
...
local function potatoRestore()
	...
	for inst,info in pairs(potatoSnapshot.particles) do if inst.Parent then pcall(function() inst.Rate=info.rate; inst.TimeScale=info.timeScale; inst.Lifetime=info.lifetime end) end end
	for inst,info in pairs(potatoSnapshot.trails) do if inst.Parent then pcall(function() inst.Lifetime=info.lifetime; inst.Enabled=info.enabled end) end end
	for inst,info in pairs(potatoSnapshot.beams) do if inst.Parent then pcall(function() inst.Segments=info.segments; inst.Enabled=info.enabled end) end end
	for inst,info in pairs(potatoSnapshot.fires) do if inst.Parent then pcall(function() inst.Heat=info.heat; inst.Size=info.size; inst.Enabled=info.enabled end) end end
	for inst,info in pairs(potatoSnapshot.smokes) do if inst.Parent then pcall(function() inst.Opacity=info.opacity; inst.Size=info.size; inst.Enabled=info.enabled end) end end
	for inst,info in pairs(potatoSnapshot.sparkles) do if inst.Parent then pcall(function() inst.Enabled=info.enabled end) end end
	for inst,enabled in pairs(potatoSnapshot.postEffects) do if inst.Parent then pcall(function() inst.Enabled=enabled end) end end
	for inst,info in pairs(potatoSnapshot.lights) do if inst.Parent then pcall(function() inst.Enabled=info.enabled end) end end
	for inst,info in pairs(potatoSnapshot.textures) do if inst.Parent then pcall(function() inst.Transparency=info.transparency end) end end
	for inst,info in pairs(potatoSnapshot.baseParts) do if inst.Parent then pcall(function() inst.Material=info.material; inst.Reflectance=info.reflectance; inst.CastShadow=info.castShadow end) end end
	for inst,fidelity in pairs(potatoSnapshot.meshRenderFidelity) do if inst.Parent then pcall(function() inst.RenderFidelity=fidelity end) end end
	if potatoSnapshot.globalShadows~=nil then pcall(function() Lighting.GlobalShadows=potatoSnapshot.globalShadows end) end
	if potatoSnapshot.lightingTechnology~=nil then pcall(function() Lighting.Technology=potatoSnapshot.lightingTechnology end) end
